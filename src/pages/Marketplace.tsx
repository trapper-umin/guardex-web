...
const Marketplace: React.FC = () => {
  ...
  const handlePageChange = (newPage: number) => {
    ...
    setTimeout(() => {
      ...
      // Прокрутка к началу результатов на мобильных
      if (isMobile) {
      ...
      <div className="flex-1">
        ...
        {isLoading ? (
          ...
          <>
            ...
            {/* Результаты */}
            <div 
              className={`results-container grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 transition-all duration-300 ${
            ...
            {/* Пагинация */}
            {totalPages > 1 && (
              <div className={`flex justify-center items-center mt-8 ${isMobile ? 'space-x-1' : 'space-x-3'}`}>
                {/* Кнопка "Назад" */}
                <button
                  onClick={() => handlePageChange(Math.max(1, currentPage - 1))}
                  disabled={currentPage === 1 || isPageChanging}
                  className={`${isMobile ? 'px-3 py-2' : 'px-6 py-3'} rounded-xl font-semibold transition-all duration-300 ${
                    currentPage === 1 || isPageChanging
                      ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                      : 'bg-white hover:bg-gray-50 text-gray-700 shadow-lg hover:shadow-xl'
                  }`}
                >
                  {isMobile ? '←' : <><span className="hidden sm:inline">← </span>Назад</>}
                </button>
                ...
                <div className={`flex items-center ${isMobile ? 'space-x-1' : 'space-x-2'}`}>
                  ...
                  {visiblePages.map((page) => (
                    <button
                      ...
                      disabled={isPageChanging}
                      className={`${isMobile ? 'px-3 py-2 text-sm' : 'px-4 py-3'} rounded-xl font-semibold transition-all duration-300 ${
                        currentPage === page
                ...
                >
                  {isMobile ? '→' : <>Вперед<span className="hidden sm:inline"> →</span></>}
                </button>
              </div>
            )}
